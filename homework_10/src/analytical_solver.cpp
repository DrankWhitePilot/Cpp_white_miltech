    ...
    double horizontalDistance)
{
    ...
    plan.fallTime = fallTime;
    plan.horizontalDistance = horizontalDistance;
    plan.targetNow = target.pos;
    plan.targetVelocity = target.velocity;
    plan.predictionUncertainty = 0.0;
    ...
    plan.totalTime = provisionalReleaseTime + fallTime;
    plan.predictedTarget = target.pos + target.velocity * plan.totalTime;
    plan.impactTarget = target.pos + target.velocity * fallTime;
    plan.dropPlan = calculateDynamicDropPlan(
        config,
        drone,
        plan.impactTarget,
    ...
    if (plan.dropPlan.needManeuver)
    {
        ...
        plan.totalTime = candidate.timeToRelease + fallTime;
        plan.predictedTarget = target.pos + target.velocity * plan.totalTime;
        candidate = findBestManeuverCandidate(
            config,
            drone,
    ...
    plan.totalTime = plan.timeToDrop + fallTime;
    return plan;
}
}
