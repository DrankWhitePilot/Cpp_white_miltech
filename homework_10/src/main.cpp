...
int main(int argc, char* argv[])
{
    ...
    bool useTable = !(argc >= 3 && std::string(argv[2]) == "analytical");
    ...
    auto targets = std::make_unique<ThreadSafeTargetProvider>(targetsPath);
    ThreadSafeTargetProvider* targetProvider = targets.get();
    ...
    DronePhysics* physicsRaw = physics.get();
    ...
    std::thread providerThread(&ThreadSafeTargetProvider::run, targetProvider);
    std::thread physicsThread(&DronePhysics::run, physicsRaw);
    std::thread missionThread(&MissionProcessor::run, &mission);

    while (!targetProvider->isThreadReady() ||
           !physicsRaw->isThreadReady() ||
