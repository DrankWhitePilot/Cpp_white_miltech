...
struct Args
{
    ...
    int startLine = 24;
    int dropLine = 23;
    std::string mavlinkAddress = "127.0.0.1";
...
bool parseArgs(int argc, char* argv[], Args& args)
{
    try {
        for (int i = 1; i < argc; ++i) {
            ...
            } else if (key == "--mavlink-address" && i + 1 < argc) {
                args.mavlinkAddress = argv[++i];
            } else if (key == "--mavlink-port" && i + 1 < argc) {
                const int port = std::stoi(argv[++i]);
                if (port < 1 || port > 65535) {
                    std::cerr << "MAVLink port must be in range 1..65535\n";
                    return false;
                }
                args.mavlinkPort = static_cast<uint16_t>(port);
            } else {
...
int main(int argc, char* argv[])
{
    ...
    if (!parseArgs(argc, argv, args)) {
        std::cerr << "Usage: mission_mavlink_udp --uart /tmp/ttyA "
                  << "--gpiochip gpiochip0 --start-line 24 --drop-line 23 "
                  << "[--mavlink-address 127.0.0.1] "
                  << "[--mavlink-port 14550]\n";
        return 1;
    }
    ...
    MavlinkUdp mavlink(args.mavlinkAddress, args.mavlinkPort);
    if (!mavlink.openSocket()) {
        return 1;
    }
    ...
    while (running) {
        ...
        if (packet.type == dlink::PKT_TELEMETRY) {
            ...
            if (copyPayload(packet, telemetry)) {
                if (!mavlink.sendTelemetry(telemetry)) {
                    return 1;
                }
                ...
                const bool newDrop = mission.drop && !previousDrop;
                if (newDrop) {
                    if (!mavlink.startDropCommand(telemetry)) {
                        return 1;
                    }
