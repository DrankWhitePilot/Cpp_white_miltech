    ...
    const AttackPlan& currentPlan)
{
    ...
    if (drone.state == DroneState::TURNING)
    {
    ...
    if (drone.state == DroneState::STOPPED ||
    ...
    AttackPlan& bestPlan) const
{
    ...
    AttackPlan* plans = new AttackPlan[targetCount]{};
    bool* validPlans = new bool[targetCount]{};
    ...
    if (currentTargetIndex >= 0 &&
        ...
        (drone_.state == DroneState::DECELERATING ||
         drone_.state == DroneState::TURNING))
    {
...
{
    if (steps_ == nullptr || stepCount_ >= MAX_STEPS)
    {
        finished_ = true;
        return false;
    }
...
void MissionProcessor::step()
{
            ...
            currentAimPoint))
    {
        ...
        bool nextStepIsBetter =
            velocityIsObserved &&
            nextDrone.state == DroneState::MOVING &&
        ...
        if (nextStepIsBetter && stepCount_ < MAX_STEPS)
        {
            ...
            if (stepCount_ + 1 < MAX_STEPS)
            {
                ...
                if (!candidateSecondDropPlan.needManeuver &&
                    secondDrone.state == DroneState::MOVING &&
                    secondDrone.speed >=
